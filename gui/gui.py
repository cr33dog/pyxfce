...
def spawn_command_line(command_line, in_terminal = False, use_startup_notification = True, screen = None, environment = None):
  ...
  return spawn_xyz(lambda x: spawn_command_line_cb(command_line + " &", x), use_startup_notification, screen, environment)

def spawn_args(args, use_startup_notification = True, screen = None, environment = None):
  return spawn_xyz(lambda x: os.spawnle(os.P_NOWAIT, args[0], args, x), use_startup_notification, screen, environment)
